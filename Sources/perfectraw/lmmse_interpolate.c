...
//  t1 = clock();
  ...
  glut[ii] = 1.055*pow(v0,1./2.4) - 0.055; } }
  // indices
  w1 = cc1;
  w2 = 2*w1;
  w3 = 3*w1;
  ...
  if (gamma_apply)
    rix[0][4] = glut[m_Image[row*m_Width+col][FC(row,col)]];
  else
    rix[0][4] = (double)m_Image[row*m_Width+col][FC(row,col)]/65535.0;
      else
  rix[0][4] = 0; }
  // G-R(B)
  ...
  for (rr=0; rr < rr1; rr++)
    for (cc=0, row=rr-ba; cc < cc1; cc++) {
      ...
      if ((row >= 0) & (row < m_Height) & (col >= 0) & (col < m_Width))
  if (gamma_apply)
    rix[0][c] = glut[m_Image[row*m_Width+col][c]];
  else
    rix[0][c] = (double)m_Image[row*m_Width+col][c]/65535.0;
      else
  rix[0][c] = 0;
  ...
  for (row=0; row < m_Height; row++)
    for (col=0, rr=row+ba; col < m_Width; col++) {
      cc = col+ba;
