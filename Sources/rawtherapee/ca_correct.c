...
//#define border2 16
  ...
  static const float clip_pt =  1.0 / MIN(MIN(m_D65Multipliers[0],m_D65Multipliers[1]),m_D65Multipliers[2]);
    ...
    for (left=-border, hblock=1; left < width; left += TS-border2, hblock++) {
      ...
      for (rr=rrmin; rr < rrmax; rr++)
        for (row=rr+top, cc=ccmin; cc < ccmax; cc++) {
          ...
          rgb[indx1][c] = m_Image[indx][FC(rr,cc)]/65535.0f;//for dcraw implementation
        }
      ...
      if (rrmax<rr1) {
        for (rr=0; rr<border; rr++)
          for (cc=ccmin; cc<ccmax; cc++) {
            ...
            rgb[(rrmax+rr)*TS+cc][c] = (m_Image[(height-rr-2)*width+left+cc][FC(rr,cc)])/65535.0f;//for dcraw implementation
          }
      }
      if (ccmin>0) {
        for (rr=rrmin; rr<rrmax; rr++)
      ...
      if (ccmax<cc1) {
        for (rr=rrmin; rr<rrmax; rr++)
          for (cc=0; cc<border; cc++) {
            ...
            rgb[rr*TS+ccmax+cc][c] = (m_Image[(top+rr)*width+(width-cc-2)][FC(rr,cc)])/65535.0f;//for dcraw implementation
          }
      }
      ...
      if (rrmin>0 && ccmin>0) {
        for (rr=0; rr<border; rr++)
          for (cc=0; cc<border; cc++) {
            ...
            // TODO: here was rgb[(border2-rr)*TS+(border2-cc)][c]
            rgb[(rr)*TS+cc][c] = (m_Image[(border2-rr)*width+(border2-cc)][FC(rr,cc)]);//for dcraw implementation
          }
      }
      if (rrmax<rr1 && ccmax<cc1) {
        for (rr=0; rr<border; rr++)
          for (cc=0; cc<border; cc++) {
            ...
            rgb[(rrmax+rr)*TS+ccmax+cc][c] = (m_Image[(height-rr-2)*width+(width-cc-2)][FC(rr,cc)])/65535.0f;//for dcraw implementation
          }
      }
      if (rrmin>0 && ccmax<cc1) {
        for (rr=0; rr<border; rr++)
          for (cc=0; cc<border; cc++) {
            ...
            rgb[(rr)*TS+ccmax+cc][c] = (m_Image[(border2-rr)*width+(width-cc-2)][FC(rr,cc)])/65535.0f;//for dcraw implementation
          }
      }
      if (rrmax<rr1 && ccmin>0) {
        for (rr=0; rr<border; rr++)
          for (cc=0; cc<border; cc++) {
            ...
            rgb[(rrmax+rr)*TS+cc][c] = (m_Image[(height-rr-2)*width+(border2-cc)][FC(rr,cc)])/65535.0f;//for dcraw implementation
          }
      }
      ...
      for (rr=4; rr < rr1-4; rr++)
        for (cc=4+(FC(rr,2)&1), indx=rr*TS+cc, c = FC(rr,cc),c = c==3?1:c; cc < cc1-4; cc+=2, indx+=2) {
      ...
      for (rr=8; rr < rr1-8; rr++)
        for (cc=8+(FC(rr,2)&1), indx=rr*TS+cc, c = FC(rr,cc),c = c==3?1:c; cc < cc1-8; cc+=2, indx+=2) {
  ...
  for (top=-border, vblock=1; top < height; top += TS-border2, vblock++)
    for (left=-border, hblock=1; left < width; left += TS-border2, hblock++) {
      ...
      for (rr=4; rr < rr1-4; rr++)
        for (cc=4+(FC(rr,2)&1), c = FC(rr,cc),c = c==3?1:c; cc < cc1-4; cc+=2) {
      ...
      for (rr=8; rr < rr1-8; rr++)
        for (cc=8+(FC(rr,2)&1), c = FC(rr,cc),c = c==3?1:c, indx=rr*TS+cc; cc < cc1-8; cc+=2, indx+=2) {
      ...
      for (rr=border; rr < rr1-border; rr++)
        for (row=rr+top, cc=border+(FC(rr,2)&1); cc < cc1-border; cc+=2) {
          col = cc + left;
          indx = row*width + col;
          c = FC(row,col);c = c==3?1:c;

          //ri->data[row][col] = CLIP((int)(65535.0f*rgb[(rr)*TS+cc][c] + 0.5f));
          if (c==1)
            m_Image[indx][1] = m_Image[indx][3] = CLIP((int)(65535.0*rgb[(rr)*TS+cc][c] + 0.5));//for dcraw implementation
          else
            m_Image[indx][c] = CLIP((int)(65535.0*rgb[(rr)*TS+cc][c] + 0.5));//for dcraw implementation
        }
